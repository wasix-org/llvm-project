; This test checks `generaldynamic` TLS support on WASIX, which (like
; Emscripten, see tls-general-dynamic.ll) supports dynamic linking with
; threads and so honors the module's TLS model instead of forcing
; `localexec`. Both WASIX v1 (wasix) and v2 (wasixv2) behave the same way.
...
; RUN: llc < %s -mtriple=wasm32-unknown-wasixv2 -asm-verbose=false -disable-wasm-fallthrough-return-opt -wasm-disable-explicit-locals -mattr=+bulk-memory,atomics | FileCheck %s --check-prefixes=CHECK,TLS
; RUN: llc < %s -mtriple=wasm32-unknown-wasixv2 -asm-verbose=false -disable-wasm-fallthrough-return-opt -wasm-disable-explicit-locals -mattr=+bulk-memory,atomics -fast-isel | FileCheck %s --check-prefixes=CHECK,TLS
...
target triple = "wasm32-unknown-wasix"
